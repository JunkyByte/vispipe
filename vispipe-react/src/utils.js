...
const TextInput = require("pixi-textinput-v5")

export function onDragStart(event)  // TODO: This function can be refactored with better logic
{
    var i;
    var obj;
    ...
    for (i=0; i<event.target.children.length; i++){
        ...
        if (this.child.type !== undefined && this.child.containsPoint(event.data.global)) {
            ...
            this.ischild = obj;
            APP.app.stage.addChildAt(this.ischild, APP.app.stage.children.length);
            break;
        }
    }
...
export function onDragEnd(event)
{
    ...
    if (this.ischild){
        ...
        if (target_conn && target_conn.type !== this.child.type){
            ...
            APP.app.stage.addChildAt(line, APP.app.stage.children.length);
            update_line(line, this.start_pos, event.data.global);
            APP.app.stage.removeChild(this.ischild)  // Delete temp line
            APP.pipeline.add_connection(output_node.id, output.index,  // TODO: This is not checked server side but client side
...
export function create_connection(input, output){
...
export function clear_connection(input){
    if (input.connection){
...
export function point_to_conn(point){
    var i, child;
    var root_obj = APP.app.renderer.plugins.interaction.hitTest(point);
...
export function update_all_lines(node){
    ...
    for (i=0; i<node.in_c.length; i++){
        if (node.in_c[i].conn_line){
            ...
            APP.app.stage.setChildIndex(node.in_c[i].conn_line, APP.app.stage.children.length-1);
        }
    }
    ...
    for (i=0; i<node.out_c.length; i++){
        for (j=0; j<node.out_c[i].conn_line.length; j++){
            if (node.out_c[i].conn_line[j]){ 
                ...
                APP.app.stage.setChildIndex(node.out_c[i].conn_line[j], APP.app.stage.children.length-1);
            }
        }
    }
}

export function update_line(line, from, to){
    line.clear();
...
export function name_to_size(name){
    var w = Math.max(60, 25 + name.length * 8);
    var h = 40; //name.length * 20;
    return [w, h];
}

export function draw_rect(width, height, color, scale){
    var obj = new PIXI.Graphics();
...
export function draw_block(name){
    var [width, height] = name_to_size(name);
    var obj = draw_rect(width, height, APP.BLOCK_COLOR, 1);
...
export function draw_text(text, scale=1){
    text = new PIXI.Text(text,
        {
            fontFamily: APP.FONT,
            fontSize: APP.FONT_SIZE * scale,
            fill: APP.TEXT_COLOR,
            align: 'right'
        });
    return text;
...
export function draw_text_input(default_value, scale=1){
    var obj = new TextInput.TextInput({
        input: {
            fontFamily: APP.FONT,
            fontSize: APP.FONT_SIZE * scale,
            padding: '12px',
...
export function draw_conn(inputs, outputs, rect){
